...
\begin{columns}[]
  \begin{column}{0.3\textwidth}
\input{figlat/latminimum}
  \end{column}
  \begin{column}{0.7\textwidth}
  Norm of a vector $\vec{x}$ is given by $\|\vec{x}\| := \sqrt{\sum x^2_i}$
  \begin{block}{First lattice minimum}
 	\begin{itemize}
 	\item Length of a shortest vector of a $n$-dimensional lattice $\Ls$:
 	\begin{eqnarray*} 
 	\lambda_1(\Ls)% &= &\min\limits_{x,y\in \Ls, x\neq y} \|x-y\|\\
 	&:= &\min\limits_{x\in \Ls, x\neq 0} \|x\|
 		\end{eqnarray*} 	
 			\item Minimal distance between two lattice vectors
 				\end{itemize}
 				\end{block}
 \end{column}
\end{columns}
...
\begin{columns}[]
  ...
  \begin{column}{0.7\textwidth}
 	\begin{itemize}
 	\item Shortest vector problem ($SVP_2$) \\
 	Given $B$, find $v\in \Ls\backslash\{0\}$ of minimal length.\vspace{.3cm}
 	\pause 
 	\item Problem is hard for large dimensions and a skewed??????????? basis 
	\end{itemize}
 \end{column}
\end{columns}
}
