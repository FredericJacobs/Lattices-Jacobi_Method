...
%    \draw [thin, gray,-latex] (YAxisMin) -- (YAxisMax);% Draw y axis
    ...
    \clip (-2.7,-3) rectangle (4cm,4.2cm); % Clips the picture...
    \pgftransformcm{1}{.9}{0.7}{1}{\pgfpoint{0cm}{0cm}}
   ...
   \foreach \x in {-14,-12,...,14}{% Two indices running over each
      \foreach \y in {-14,-12,...,14}{% node on the grid we have drawn 
        \node[draw,circle,inner sep=1.2pt,fill=mygreen] at (\x,\y) {};  }}
         ...
         \node[draw,circle,inner sep=1.4pt,fill=myred] at (-2,4) {}; 
    \node[draw,circle,inner sep=6.5pt] at (0,0) {};
      \node[draw,circle,inner sep=24pt, dashed] at (0,0) {};
      %\draw [thin, gray,-latex] (0,0) -- (3.5,-1) node [below, node distance =0.2cm] {} ;
   \draw [thin, gray] (0,0) -- (-2,2) node [below, node distance =0.2cm] {} ;
   \draw [thin, gray] (0,0) -- (-2,4) node [below, node distance =0.2cm] {} ;
   \node (b) at (-1.7,1.4) {$\lambda_1$} ;
     \node (b) at (-2,4.5) {$\lambda_2$} ;
